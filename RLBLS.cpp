...
long long ToFixedInt(double value, int precision) {
...
void ReadingMDPFile(string input_file) {
	...
	data >> ver_num >> k_num;
	// 时间限制可以按需要保留或修改
	if (ver_num == 2500) { time_limit = 300; }
	else if (ver_num == 3001) { ver_num--; time_limit = 600; }
	else if (ver_num == 5001) { ver_num--; time_limit = 1800; }
	//time_limit = 17;
	...
	int min_node = max_value;
	int max_node = min_value;

	while (data >> x1 >> x2 >> x3) {
		vec_x1.push_back(x1);
		vec_x2.push_back(x2);
		vec_w.push_back(x3);
		if (x1 < min_node) min_node = x1;
		if (x2 < min_node) min_node = x2;
		if (x1 > max_node) max_node = x1;
		if (x2 > max_node) max_node = x2;
	}

	// 自动判断是否为 1-based 编号
	bool one_based = false;
	if (max_node == ver_num) { one_based = true; }

	int sumedge = 0;
	for (int i = 0; i < (int)vec_x1.size(); i++) {
		x1 = vec_x1[i];
		x2 = vec_x2[i];
		x3 = vec_w[i];
		if (one_based) { x1--; x2--; }
...
void ReadingCliqueFile(string input_file) {
	ifstream data(input_file);
	if (!data.is_open()) {
		cout << "Can not open the instance " << input_file << endl;
		exit(0);
	...
	while (getline(data, line)) {
		...
		// p 行：p ver_num eg_num k_num
		if (tag == "p") {
			ss >> ver_num >> declared_edge_num >> k_num;
			eg_num = declared_edge_num;
			// 分配矩阵
			edge = new int* [ver_num];
			weight = new double* [ver_num];
			for (int i = 0; i < ver_num; i++) {
		...
		if (tag == "e") {
			if (!graph_initialized) {
				cout << "### Error: edge line appears before p line." << endl;
				exit(0);
			}
...
void Save_bestsolu() {
	...
	for (int i = 0; i < ver_num - 1; i++) {
		for (int j = i + 1; j < ver_num; j++) {
			if (best_solu[i] == 1 && best_solu[j] == 1) {
				test_obj += weight[i][j];
			}
		}
	}
	cout << "test_obj = " << test_obj << endl << endl;*/
}
...
void SetVector() {
	...
	for (int i = 0; i < ver_num; i++) {
		rand_vec[i] = rand() % 131073;  // 生成 0 到 131072 之间的整数
	}
}
...
void Move(int out_ver, int in_ver) {
	...
	H[out_ver] = Iter;
	H[in_ver] = Iter;
	int rand_range = static_cast<int>(ceil(tabup2 * k_num));
	if (rand_range <= 0) {
		tabu_len = static_cast<int>(ceil(tabup1 * k_num));
	}
	else {
		tabu_len = static_cast<int>(ceil(tabup1 * k_num)) + rand() % rand_range;
	}
...
void Local_Search() {
	...
	do {
		...
		for (int i = 0; i < k_num; i++) {
			if (dminout > a[vertex[i]]) {
				dminout = a[vertex[i]]; verout = vertex[i];
			}
		}
		for (int i = k_num; i < ver_num; i++) {
			if (dmaxin < a[vertex[i]]) {
				dmaxin = a[vertex[i]]; verin = vertex[i];
			}
		}
		int xnum = 0, ynum = 0;
		for (int i = 0; i < k_num; i++) {
			if (a[vertex[i]] <= dminout + weight[verout][verin]) {
				x[xnum] = vertex[i]; xnum++;
			}
		}
		for (int i = k_num; i < ver_num; i++) {
			if (a[vertex[i]] >= dmaxin - weight[verout][verin]) {
				y[ynum] = vertex[i]; ynum++;
			}
		}
		cur_obj = obj;
		int improve = 0;
	...
	} while (ToFixedInt(obj, precision) > ToFixedInt(cur_obj, precision));
}
...
void ini_twoswap() {
	...
	do {
		...
		for (int i = k_num; i < ver_num - 1; i++) {
			for (int j = i + 1; j < ver_num; j++) {
				int ingain = a[vertex[i]] + a[vertex[j]] + weight[vertex[i]][vertex[j]];
				if (ingain > maxsum) {
					maxsum = ingain; verc = vertex[j]; verd = vertex[i];
				}
			}
		}
		for (int i = 0; i < k_num - 1; i++) {
			for (int j = i + 1; j < k_num; j++) {
				int outgain = a[vertex[i]] + a[vertex[j]] - weight[vertex[i]][vertex[j]];
				if (outgain < minsum) {
					minsum = outgain; vera = vertex[j]; verb = vertex[i];
				}
			}
		}
		int db = weight[vera][verd] + weight[verb][verc] + weight[vera][verc] + weight[verb][verd];
		int xnum = 0, ynum = 0;
		for (int i = 0; i < k_num - 1; i++) {
			for (int j = i + 1; j < k_num; j++) {
				if (a[vertex[i]] + a[vertex[j]] - weight[vertex[i]][vertex[j]] <= minsum + db) {
					//if (fabs(a[vertex[i]] + a[vertex[j]] - weight[vertex[i]][vertex[j]] - (minsum + db)) <= 1e-6) {
					xts1[xnum] = vertex[i];
					xts2[xnum] = vertex[j]; xnum++;
				}
			}
		}
		for (int i = k_num; i < ver_num - 1; i++) {
			for (int j = i + 1; j < ver_num; j++) {
				if (a[vertex[i]] + a[vertex[j]] + weight[vertex[i]][vertex[j]] >= maxsum - db) {
					//if (fabs(a[vertex[i]] + a[vertex[j]] + weight[vertex[i]][vertex[j]] - (maxsum - db)) <= 1e-6) {
						//cout << "ynum = "<<ynum << endl;
					yts1[ynum] = vertex[i];
					yts2[ynum] = vertex[j]; ynum++;
				}
			}
		}
		cur_obj = obj;
...
void ini_DirectPerturbationattr2dmax() {
	...
	int xnum = 0, ynum = 0;
	for (int i = 0; i < k_num; i++) {
		if (a[vertex[i]] <= dminout + dmaxfortabu * 2.0) {
			x[xnum] = vertex[i]; xnum++;
		}
	}
	for (int i = k_num; i < ver_num; i++) {
		if (a[vertex[i]] >= dmaxin - dmaxfortabu * 2.0) {
			y[ynum] = vertex[i]; ynum++;
		}
	}
	double improve = 0;
		...
		for (int j = 0; j < ynum; j++) {
			...
			if (tabu_list[y[j]] < Iter && tabu_list[x[i]] < Iter) {
				if (improve > max) {
					...
					if (num_to_in < ver_num) { // 防护屏障：不能超过数组实际容量
						to_in[num_to_in] = y[j];
						to_out[num_to_out] = x[i];
						num_to_in++; num_to_out++;
					}
...
void RandomPerturbation(int L) {
	int x1 = 0, x2 = 0;
	for (int i = 0; i < ver_num; i++) {
		if (solu[i] == 1) { out_list[x1] = i; x1++; }
		else { in_list[x2] = i; x2++; }
	}
	RandSort(out_list, x1); // 原本是 k_num，必须改为真实的 x1
	RandSort(in_list, x2);  // 必须改为真实的 x2

	int limit = min(L, min(x1, x2)); // 确保绝不越界
	int y1, y2;
...
void MemoryPerturbation(int L) {
	// 防止同一次扰动中同一个点被反复换入换出
	vector<int> used(ver_num, 0);
	int move_cnt = 0;
	while (move_cnt < L) {
		int out_ver = -1;
		int in_ver = -1;
		double best_out_score = -1e100;
		double best_in_score = -1e100;
		// 1. Compute scores and select one out-vertex and one in-vertex
		for (int i = 0; i < ver_num; i++) {
			if (used[i]) { continue; }
			double elite_part = 0.0;
			double flip_part = 0.0;
			if (cur_elite_num > 0) {
				elite_part =
					(double)elitefreq[i] *
					((double)cur_elite_num - (double)elitefreq[i]) /
					((double)cur_elite_num * (double)cur_elite_num);
			}
			if (max_freq > 0) {
				flip_part = 0.3 * (1.0 - (double)flipfreq[i] / (double)max_freq);
			}
			else {
				flip_part = 0.3;
			}
			double cur_score = elite_part + flip_part;
			// selected vertex: candidate to be removed
			if (solu[i] == 1) {
				if (cur_score > best_out_score) {
					best_out_score = cur_score;
					out_ver = i;
				}
			}
			// unselected vertex: candidate to be inserted
			if (solu[i] == 0) {
				if (cur_score > best_in_score) {
					best_in_score = cur_score;
					in_ver = i;
				}
			}
		}
		// 没有合法 swap，则停止
		if (out_ver == -1 || in_ver == -1) {
			break;
		}

		Move(out_ver, in_ver);
		// 标记本次扰动中已经使用过的点，避免立即反向换回
		used[out_ver] = 1;
		used[in_ver] = 1;
		move_cnt++;
	}
...
bool RemoveOnly(int out_ver, int& cur_size) {
	if (solu[out_ver] == 0) return false;
	// 1. 更新目标值
	obj -= a[out_ver];
	// 2. 更新 a[]
	for (int v = 0; v < ver_num; ++v) {
		a[v] -= weight[v][out_ver];
	}
	// 3. 更新 solu
	solu[out_ver] = 0;
	// 4. 更新 tabu / history
	...
	tabu_list[out_ver] = Iter + tabu_len;
	// 5. 更新 hash
	hashvalue -= rand_vec[out_ver];
	// 6. 更新 vertex 和 address
	int pos_out = address[out_ver];
	int pos_last_selected = cur_size - 1;
	int last_selected_ver = vertex[pos_last_selected];
	vertex[pos_out] = last_selected_ver;
	vertex[pos_last_selected] = out_ver;
	address[last_selected_ver] = pos_out;
	address[out_ver] = pos_last_selected;
	cur_size--;
	// 7. 更新 flipfreq
	flipfreq[out_ver]++;
	if (flipfreq[out_ver] > max_freq) {
		max_freq = flipfreq[out_ver];
	}
	Iter++;
	return true;
}

bool AddOnly(int in_ver, int& cur_size) {
	if (solu[in_ver] == 1) return false;
	obj += a[in_ver];
	for (int v = 0; v < ver_num; ++v) {
		a[v] += weight[v][in_ver];
	}
	...
	tabu_list[in_ver] = Iter + ceil(0.7 * tabu_len);
	// 5. 更新 hash
	hashvalue += rand_vec[in_ver];
	// 6. 更新 vertex 和 address
	int pos_in = address[in_ver];
	int pos_first_unselected = cur_size;
	int first_unselected_ver = vertex[pos_first_unselected];
	vertex[pos_first_unselected] = in_ver;
	vertex[pos_in] = first_unselected_ver;
	address[in_ver] = pos_first_unselected;
	address[first_unselected_ver] = pos_in;
	cur_size++;
	// 7. 更新 flipfreq
	flipfreq[in_ver]++;
	if (flipfreq[in_ver] > max_freq) {
		max_freq = flipfreq[in_ver];
	}
	Iter++;
	return true;
}

void GernalSwap(int L) {
	vector<bool> destroyed(ver_num, false);
	vector<bool> inserted(ver_num, false);
	int cur_size = k_num;
	int destroy_cnt = 0;
	// ==============================
	// 1. Destroy phase: remove L vertices
	// ==============================
	while (destroy_cnt < L) {
		double min_a = max_value;
		int outver = -1;
		for (int j = 0; j < ver_num; ++j) {
			if (solu[j] == 1 && !destroyed[j]) {
				if (a[j] < min_a) {
					min_a = a[j];
					outver = j;
				}
		...
		if (outver == -1) break;
		destroyed[outver] = true;
		RemoveOnly(outver, cur_size);
		destroy_cnt++;
	}
	int reconstruct_cnt = 0;

	// 2. Reconstruct phase: insert L vertices
	while (reconstruct_cnt < destroy_cnt) {
		double max_a = min_value;
		int inver = -1;
		for (int j = 0; j < ver_num; ++j) {
			// destroyed[j] == true 的点不允许马上加回来
			// 这样才能保证 perturbation 真正替换掉 L 个 selected vertices
			if (solu[j] == 0 && !destroyed[j] && !inserted[j]) {
				if (a[j] > max_a) {
					max_a = a[j];
					inver = j;
				}
...
void Compute_Reward(int prevst0, int prevper, int curst1, int curst2) {
	if (prev_visit == -1) {
		Reward[prevst0][prevper] += reward1;//新解强奖励
		Reward[prevst0][prevper] += (curst2 - reward2);//new
		if (prevper == 0) { Reward[prevst0][prevper] += 4; }
		else if (prevper == 1) { Reward[prevst0][prevper] += 3; }
		else if (prevper == 2) { Reward[prevst0][prevper] += 1; }
		else if (prevper == 3) { Reward[prevst0][prevper] += 2; }
	}
	else {
		if (curst1 == 1) { Reward[prevst0][prevper] -= 2; }
		else if (curst1 == 2) { Reward[prevst0][prevper] -= 3; }
		else if (curst1 == 3) { Reward[prevst0][prevper] -= 4; }
		else if (curst1 == 4) { Reward[prevst0][prevper] -= 5; }
		Reward[prevst0][prevper] += (curst2 - reward2);//new
	}
}
...
void DeterminePerturbationType() {
	...
	if (perturb_num > 1) {
		...
		SetState2(cur_st2);
		//先计算上一轮(st0, per)的reward
		Compute_Reward(prev_st0, prev_per, cur_st1, cur_st2);
		//更新Q_table
		Q_table[prev_st0][prev_per] = (1 - alpha) * Q_table[prev_st0][prev_per];
		Q_table[prev_st0][prev_per] += (alpha * Reward[prev_st0][prev_per]);
		//确定当前状态
		prev_st0 = cur_st1 * St2_Num + cur_st2;
		double nextmaxQ = 0;
		for (int i = 0; i < Pertype_num; i++) {
			if (i == 0) { nextmaxQ = Q_table[prev_st0][i]; }
			else {
				if (nextmaxQ < Q_table[prev_st0][i]) { nextmaxQ = Q_table[prev_st0][i]; }
			}
		}
		Q_table[prev_st0][prev_per] += (alpha * gamma1 * nextmaxQ);
	}
	...
	if (prob <= sigma) {
		...
		for (int i = 0; i < Pertype_num; i++) {
			if (i == 0) { nextmaxQ = Q_table[prev_st0][i]; prev_per = i; }
				...
				if (nextmaxQ < Q_table[prev_st0][i]) { nextmaxQ = Q_table[prev_st0][i]; prev_per = i; }
					...
					double prob1 = static_cast<double>(rand()) / static_cast<double>(RAND_MAX);
		...
		//随机选
		prev_per = rand() % Pertype_num;
	}
	int curL = L;
	if (prev_per == 0) {
		sum0++;
		para = 1;
		for (int i = 0; i < para * curL; i++) {
			DirectPerturbationattr2dmax();
			if (ToFixedInt(obj, precision) > ToFixedInt(best_obj, precision)) {
				finishing_time = (double)clock();
				Save_bestsolu();
			}
		}
...
void Breakout_Local_Search(int time_limit) {
	lc = 0;
	wc = 20.0;
	L = L_MIN;
	prev_visit = 0;
	  ...
	  //different in mdp and hsp
		if ((diff <= 0.5 * sumdiff / (double)countdiff) || diff < 0) {
			do {
				...
				if (fabs(obj - ini_obj) <= 1e-6) { break; }
				if (((double)clock() - starting_time) / CLOCKS_PER_SEC > time_limit) { break; }
			} while (ToFixedInt(obj, precision) > ToFixedInt(ini_obj, precision));
		}
		if (ToFixedInt(obj, precision) > ToFixedInt(best_obj, precision)) {
			finishing_time = (double)clock();
			Save_bestsolu();
		...
		prev_lo = obj;
		perturb_num++;
		DetermineJumpMagnitude(); DeterminePerturbationType();
...
void ini_K_twoswap() {
	...
	do {
		...
		// 2. Compute lambda lambda = delta_ref - 4 * wmin
		double delta_ref =
...
void K_Delete() {
	// 1. 算出当前解内有多少个点
	int cur_k = 0;
	for (int i = 0; i < ver_num; i++) if (solu[i]) cur_k++;
	...
	int in_ver = vertex[cur_k - 1]; // 与解内的最后一个点交换
...
void K_Add() {
	...
	int ver_out = vertex[cur_k]; // 与解外的第一个点交换 (边界变成了cur_k)
	int x = address[ver_in];
	vertex[x] = ver_out; vertex[address[ver_out]] = ver_in;
...
int main(int argc, char** argv) {
	...
	//instance_name += ".txt";
	string resultfile = "RLBLS_COR_R1_Ablation/";
	//paras:
	...
	time_limit = 200;
	if (!file1.empty() && file1[0] == 'M') {
		cout << "MDP" << endl; ReadingMDPFile(instance_name);
		resultfile += "result/";
	}
	else if (!file1.empty() && file1[0] == 'C') {
		cout << "CLIQUE" << endl; ReadingCliqueFile(instance_name);
		resultfile += "result/";
		k_num--;
		k_num--;
		k_num--;
	...
	else if (!file1.empty() && file1[0] == 'D') {
		cout << "DKS" << endl; ReadingCliqueFile(instance_name);
		resultfile += "result/";
		if (ver_num == 800) { time_limit = 2000; }
		else { time_limit = 17; }
	}
	else if (!file1.empty() && file1[0] == 'H') {
		cout << "HCP" << endl; ReadingCliqueFile(instance_name);
		resultfile += "result/";
		time_limit = 2000;
	}
	else if (!file1.empty() && file1[0] == 'T') {
		cout << "HSP" << endl; ReadingHSPFile(instance_name);
		resultfile += "result/";
	}
	resultfile += file1;
	//resultfile += ".txt";
	...
	//Breakout_Local_Search(time_limit);
	...
	for (int i = 0; i < ver_num; i++) {
		if (best_solu[i] == 1) { cout << i + 1 << endl; }
	}
	double test_obj = 0;
	for (int i = 0; i < ver_num - 1; i++) {
		for (int j = i + 1; j < ver_num; j++) {
